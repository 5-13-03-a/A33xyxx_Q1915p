...
(function () {
    ...
    function buildSystemPrompt(ent, heatLevel) {
        ...
        return ARCHITECT_STYLE + '\n\n' +
            ...
            '- RESPONSE LENGTH (ABSOLUTE HARD LIMIT — VIOLATION = FAILURE):\n' +
            '  · You MUST write between 2 and 4 paragraphs. NEVER more than 4.\n' +
            '  · Total character count MUST NOT exceed ' + (parseInt(localStorage.getItem('lens-max-tokens') || '1200', 10)) + ' characters (NOT tokens — characters including spaces).\n' +
            '  · If you write more than 4 paragraphs or exceed the character limit, your response will be REJECTED and you will be asked to redo it.\n' +
            '  · End at a natural pause. Leave the reader wanting more. Density over length.\n' +
            '  · Count your paragraphs before outputting. If you have 5+, DELETE one.\n' +
            ...
            (function(){
                var memKey='ca-memory-'+ent.id;
                var memData;try{memData=JSON.parse(localStorage.getItem(memKey)||'{}');}catch(e){memData={};}
                var h=(memData.high||[]).slice(0,3);
                var m=(memData.mid||[]).slice(0,3);
                var l=(memData.low||[]).slice(0,3);
                if(!h.length&&!m.length&&!l.length)return '';
                var inject='\n\n[RELATIONSHIP MEMORY — You remember these facts. Embody them naturally, never recite them.]\n';
                if(h.length)inject+='CORE: '+h.join(' | ')+'\n';
                if(m.length)inject+='IMPORTANT: '+m.join(' | ')+'\n';
                if(l.length)inject+='RECENT: '+l.join(' | ')+'\n';
                return inject;
            })()+'\n';
    }
    ...
    function buildCSS() {
        return '' +
            ...
            '#lensApp .char-pin{position:absolute;top:12px;left:12px;z-index:5;width:22px;height:22px;display:flex;align-items:center;justify-content:center;pointer-events:none;}' +
            '#lensApp .char-pin svg{width:12px;height:12px;fill:rgba(255,255,255,0.7);stroke:none;filter:drop-shadow(0 1px 3px rgba(0,0,0,0.4));}' +
            '#lensApp .char-arrow{position:absolute;right:20px;bottom:22px;width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);display:flex;align-items:center;justify-content:center;z-index:5;cursor:pointer;-webkit-tap-highlight-color:transparent;transition:transform 0.2s,background 0.2s;}' +
            '#lensApp .char-arrow:active{transform:scale(0.88);background:rgba(255,255,255,.15);}' +
            ...
            '#lensApp .narr .tension:last-child{margin-bottom:0;padding-bottom:10px;}' +
            ...
            '#lensApp .lens-inject-slider::-webkit-slider-thumb{-webkit-appearance:none;width:14px;height:14px;border-radius:50%;background:#fff;cursor:pointer;box-shadow:0 0 8px rgba(255,255,255,0.3)}' +
        ...
        el.innerHTML = '<style>' + buildCSS() + '</style>' +
                ...
                '<div class="app">' +
                    ...
                    '<div class="bottom-zone">' +
                        ...
                        '<div class="bottom-glass">' +
                            ...
                            '<button class="bar-act-btn" id="lensStopBtn" style="display:none;color:rgba(240,80,80,.9);"><svg viewBox="0 0 24 24"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>Stop</button>' +
            ...
            '<div class="lens-sidebar" id="lensSb">' +
                ...
                '<div class="sb-content">' +
                    ...
                    /* ── 对话轮数 ── */
                    '<div class="lens-sb-section" id="lensRoundsCard">' +
                        '<div class="lens-sb-toggle" id="lensRoundsToggle">' +
                            '<span class="sb-label" style="margin-bottom:0;">CONTEXT ROUNDS</span>' +
                            '<svg class="lens-sb-arrow" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg>' +
                        '</div>' +
                        '<div class="lens-sb-body">' +
                            '<div style="padding-top:12px;">' +
                                '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">' +
                                    '<span style="font-family:\'Space Grotesk\',sans-serif;font-size:9px;letter-spacing:1px;color:var(--lk-tss);text-transform:uppercase;">Rounds</span>' +
                                    '<span style="font-family:\'Syncopate\',sans-serif;font-size:10px;font-weight:700;color:var(--lk-t);" id="lensRoundsVal">30</span>' +
                                '</div>' +
                                '<input type="range" class="heat-slider" id="lensRoundsSlider" min="4" max="60" step="2" value="30" style="margin-bottom:8px;">' +
                                '<div style="display:flex;justify-content:space-between;">' +
                                    '<span style="font-size:9px;color:var(--lk-tss);font-family:\'Space Grotesk\',sans-serif;">4</span>' +
                                    '<span style="font-size:9px;color:var(--lk-tss);font-family:\'Space Grotesk\',sans-serif;">60</span>' +
                                '</div>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                    ...
                    '<button class="lens-trigger-btn" id="lensSbArchive" style="margin-bottom:10px;letter-spacing:1.5px;">\u2726 ARCHIVES</button>' +
                    '<button class="lens-trigger-btn" id="lensSbNewChapter" style="margin-bottom:10px;letter-spacing:1.5px;opacity:.7;">\u2500 NEW CHAPTER</button>' +
                    '<button class="lens-trigger-btn" id="lensSbClear" style="margin-bottom:10px;letter-spacing:1.5px;opacity:.5;color:rgba(240,80,80,.7);border-color:rgba(240,80,80,.15);">\u2715 CLEAR CHAT</button>' +
                    '<button class="lens-trigger-btn" id="lensSbTriggerAI">INVOKE AI RESPONSE</button>' +
                    '<button class="lens-trigger-btn" id="lensSbSummarize" style="margin-top:10px;opacity:.7;letter-spacing:1.5px;">✦ SUMMARIZE TO MEMORY</button>' +
                '</div>' +
            '</div>';

        /* 存档面板 */
        var arcEl = document.createElement('div');
        arcEl.id = 'lensArchivePanel';
        arcEl.style.cssText = 'position:absolute;inset:0;z-index:200;background:#050810;display:none;flex-direction:column;overflow:hidden;';
        arcEl.innerHTML =
            '<div style="position:absolute;inset:0;pointer-events:none;overflow:hidden;">' +
                '<div style="position:absolute;inset:0;background-image:linear-gradient(rgba(255,255,255,0.015) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.015) 1px,transparent 1px);background-size:60px 60px;mask-image:radial-gradient(ellipse at center,black 30%,transparent 80%);-webkit-mask-image:radial-gradient(ellipse at center,black 30%,transparent 80%);"></div>' +
            '</div>' +
            '<div style="position:relative;z-index:10;padding:56px 20px 20px;display:flex;justify-content:space-between;align-items:flex-start;">' +
                '<div>' +
                    '<div style="font-family:Syncopate,sans-serif;font-size:8px;letter-spacing:4px;color:rgba(255,255,255,0.3);text-transform:uppercase;margin-bottom:8px;">Lens · Memory Vault</div>' +
                    '<div style="font-family:Playfair Display,serif;font-size:28px;font-style:italic;color:#fff;letter-spacing:1px;line-height:1;margin-bottom:4px;">Archives</div>' +
                    '<div style="font-family:Cormorant Garamond,serif;font-size:12px;font-style:italic;color:rgba(255,255,255,0.35);letter-spacing:1px;">Preserved scenes & conversations</div>' +
                '</div>' +
                '<div id="lensArcClose" style="width:38px;height:38px;border-radius:50%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);display:flex;align-items:center;justify-content:center;cursor:pointer;margin-top:4px;"><svg viewBox="0 0 24 24" style="width:14px;height:14px;stroke:rgba(255,255,255,0.6);fill:none;stroke-width:2;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div>' +
            '</div>' +
            '<div id="lensArcStats" style="position:relative;z-index:10;display:flex;gap:16px;padding:0 20px 16px;border-bottom:1px solid rgba(255,255,255,0.06);margin-bottom:16px;"></div>' +
            '<div style="position:relative;z-index:10;display:flex;gap:8px;padding:0 20px 16px;">' +
                '<div id="lensArcSave" style="padding:9px 16px;border-radius:20px;background:rgba(180,210,255,0.1);border:1px solid rgba(180,210,255,0.2);color:rgba(180,210,255,0.9);font-size:10px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:6px;transition:all 0.2s;"><svg viewBox="0 0 24 24" style="width:12px;height:12px;stroke:currentColor;fill:none;stroke-width:2;"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>Save Current</div>' +
            '</div>' +
            '<div id="lensArcScroll" style="position:relative;z-index:10;flex:1;overflow-y:auto;padding:0 16px 40px;scrollbar-width:none;"></div>';
        el.querySelector('#lensChatPage .app').appendChild(arcEl);
    ...
    function renderCharList() {
        ...
        var pinnedIds = [];
        try { pinnedIds = JSON.parse(localStorage.getItem('lens-pinned') || '[]'); } catch(e) {}
        var sorted = ents.slice().sort(function(a, b) {
            var aPin = pinnedIds.indexOf(a.id) !== -1 ? 0 : 1;
            var bPin = pinnedIds.indexOf(b.id) !== -1 ? 0 : 1;
            return aPin - bPin;
        });

        var html = '<div class="section-label">\u2014 Your Entities \u2014</div>';
        sorted.forEach(function (ent, idx) {
            var isPinned = pinnedIds.indexOf(ent.id) !== -1;
            var dispName = ent.nickname || ent.name;
            ...
            html += '<div class="char-card" data-ent-id="' + ent.id + '">' +
                ...
                (isPinned ? '<div class="char-pin"><svg viewBox="0 0 24 24"><path d="M12 2L9.5 9.5 2 12l7.5 2.5L12 22l2.5-7.5L22 12l-7.5-2.5z"/></svg></div>' : '') +
        ...
        list.querySelectorAll('.char-card').forEach(function (card) {
            ...
            card.addEventListener('touchstart', function(e) {
                ...
                _pinTimer = setTimeout(function() {
                    ...
                    var pins = [];
                    try { pins = JSON.parse(localStorage.getItem('lens-pinned') || '[]'); } catch(err) {}
                    var idx = pins.indexOf(entId);
                    if (idx !== -1) {
                        pins.splice(idx, 1);
                    } else {
                        pins.unshift(entId);
                    }
                    localStorage.setItem('lens-pinned', JSON.stringify(pins));
                    renderCharList();
                }, 600);
            }, {passive: true});
            card.addEventListener('touchend', function() { clearTimeout(_pinTimer); });
            card.addEventListener('touchmove', function() { clearTimeout(_pinTimer); });

            /* 点击箭头 → 直接进入 */
            var arrow = card.querySelector('.char-arrow');
            if (arrow) {
                arrow.addEventListener('click', function(e) {
                    e.stopPropagation();
                    var existing = document.getElementById('lensCardExpand');
            ...
            card.addEventListener('click', function (e) {
                if (_pinFired) { _pinFired = false; return; }
                if (e.target.closest('.char-arrow')) return;

                var existing = document.getElementById('lensCardExpand');
                if (existing) {
                    if (existing.dataset.entId === entId) { existing.remove(); return; }
                    existing.remove();
                }

                var ent = ents.find(function(en) { return en.id === entId; });
                if (!ent) { enterChat(entId); return; }

                var dispName = ent.nickname || ent.name;
                var savedRounds = parseInt(localStorage.getItem('lens-inject-rounds-' + entId) || '6', 10);

                var panel = document.createElement('div');
                panel.id = 'lensCardExpand';
                panel.dataset.entId = entId;
                panel.style.cssText = 'background:rgba(8,10,18,0.95);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:18px;margin-top:-8px;margin-bottom:14px;animation:lk-fadeIn 0.3s ease-out forwards;opacity:0;';

                panel.innerHTML =
                    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">' +
                        '<div style="font-family:Playfair Display,serif;font-size:16px;font-style:italic;color:#fff;">' + esc(dispName) + '</div>' +
                        '<div class="lens-expand-close" style="width:24px;height:24px;border-radius:50%;background:rgba(255,255,255,0.06);display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg viewBox="0 0 24 24" style="width:10px;height:10px;stroke:rgba(255,255,255,0.5);fill:none;stroke-width:2;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div>' +
                    '</div>' +
                    '<div style="margin-bottom:14px;">' +
                        '<div style="font-family:Space Grotesk,sans-serif;font-size:8px;letter-spacing:2px;color:rgba(255,255,255,0.3);text-transform:uppercase;margin-bottom:8px;">Chat Context Injection / 聊天室注入</div>' +
                        '<div style="font-size:10px;color:rgba(255,255,255,0.35);margin-bottom:10px;line-height:1.5;font-style:italic;">从主聊天室注入多少条消息作为背景参考。0 = 完全独立。</div>' +
                        '<div style="display:flex;align-items:center;gap:10px;">' +
                            '<input type="range" class="lens-inject-slider" min="0" max="100" value="' + savedRounds + '" style="flex:1;height:4px;background:rgba(255,255,255,0.1);border-radius:2px;outline:none;-webkit-appearance:none;" data-ent-id="' + entId + '">' +
                            '<span class="lens-inject-val" style="font-family:Syncopate,sans-serif;font-size:11px;font-weight:700;color:#fff;min-width:20px;text-align:center;">' + savedRounds + '</span>' +
                        '</div>' +
                        '<div style="display:flex;justify-content:space-between;margin-top:4px;"><span style="font-size:8px;color:rgba(255,255,255,0.2);">0 (独立)</span><span style="font-size:8px;color:rgba(255,255,255,0.2);">100 条</span></div>' +
                    '</div>' +
                    '<div class="lens-expand-enter" style="width:100%;padding:12px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:30px;text-align:center;font-family:Syncopate,sans-serif;font-size:10px;letter-spacing:2px;color:#fff;cursor:pointer;transition:all 0.2s;" data-ent-id="' + entId + '">ENTER SCENE</div>';

                card.parentNode.insertBefore(panel, card.nextSibling);

                panel.querySelector('.lens-inject-slider').addEventListener('input', function() {
                    panel.querySelector('.lens-inject-val').textContent = this.value;
                    localStorage.setItem('lens-inject-rounds-' + this.dataset.entId, this.value);
                });

                panel.querySelector('.lens-expand-close').addEventListener('click', function(e) {
                    e.stopPropagation();
                    panel.remove();
                });

                panel.querySelector('.lens-expand-enter').addEventListener('click', function(e) {
                    e.stopPropagation();
                    panel.remove();
                    enterChat(this.dataset.entId);
                });
            });
        });
    }
    ...
    function renderConvToDOM(msgs, isLoadMore) {
        ...
        visible.forEach(function(m, vIdx) {
            ...
            if (m.role === 'user' && m.isDirector) {
                ...
                html += '</div>';
                d.innerHTML = html;
            } else if (m.isChapterBreak) {
                d.className = 'msg-row';
                d.dataset.msgIdx = realIdx;
                d.dataset.msgRole = 'system';
                d.style.cssText = 'display:flex;align-items:center;gap:12px;padding:20px 24px;pointer-events:none;';
                d.innerHTML = '<div style="flex:1;height:1px;background:linear-gradient(to right,transparent,rgba(255,255,255,0.15),transparent);"></div>' +
                    '<div style="font-family:Syncopate,sans-serif;font-size:7px;letter-spacing:3px;color:rgba(255,255,255,0.25);text-transform:uppercase;white-space:nowrap;">New Chapter</div>' +
    ...
    function lensAbortAI() {
        if (_lensAbort) { _lensAbort.abort(); _lensAbort = null; }
        hideTypingIndicator();
        var stopBtn = document.getElementById('lensStopBtn');
    ...
    function callLensAI(userContent, callback) {
        ...
        _lensAbort = new AbortController();
        var stopBtn = document.getElementById('lensStopBtn');
        ...
        /* 找到最后一个 chapter break，只取之后的消息 */
        var chapterStart = 0;
        for (var ci = saved.length - 1; ci >= 0; ci--) {
            if (saved[ci].isChapterBreak) { chapterStart = ci + 1; break; }
        }
        var activeMessages = saved.slice(chapterStart);
        ...
        var contextRounds = parseInt(localStorage.getItem('lens-context-rounds') || '30', 10);
        var lastN = history.slice(-contextRounds);
        var apiMessages = [{ role: 'system', content: systemPrompt }];

        /* 注入聊天室消息作为背景上下文（按用户设置的轮数） */
        try {
            var _lensEntId = curEnt ? curEnt.id : null;
            var _injectRounds = parseInt(localStorage.getItem('lens-inject-rounds-' + _lensEntId) || '6', 10);
            if (_injectRounds > 0) {
                var _mainConv = null;
                if (_lensEntId && window._caConversations && window._caConversations[_lensEntId]) {
                    _mainConv = window._caConversations[_lensEntId];
                }
                if (_mainConv && _mainConv.length) {
                    var _lastN = _mainConv.slice(-_injectRounds);
                    var _injected = 0;
                    _lastN.forEach(function(m) {
                        if (m.role === 'user' || m.role === 'assistant') {
                            if (m.text && m.text.indexOf('[IMAGE]') !== 0) {
                                apiMessages.push({ role: m.role === 'user' ? 'user' : 'assistant', content: '[PRIOR CHAT CONTEXT] ' + m.text });
                                _injected++;
                            }
                        }
                    });
                    if (_injected > 0) {
                        apiMessages.push({ role: 'system', content: '[The above ' + _injected + ' messages are from the main chat room, provided as background context only. The immersive scene below is separate and independent.]' });
                    }
                }
            }
        } catch(e) {}
        ...
        fetch(endpoint, {
            ...
            signal: _lensAbort ? _lensAbort.signal : undefined
        })
        .then(function (res) {
        ...
        .then(function (data) {
            _lensAbort = null;
            var stopBtn2 = document.getElementById('lensStopBtn');
        ...
        .catch(function (err) {
            _lensAbort = null;
            var stopBtn3 = document.getElementById('lensStopBtn');
    ...
    function bindAll() {
        ...
        document.getElementById('lensStopBtn').addEventListener('click', lensAbortAI);
        ...
        document.getElementById('lensSbSummarize').addEventListener('click', function () {
            toggleSB();
            if (!curId || !curEnt || !curMessages.length) return;
            var btn = document.getElementById('lensSbSummarize');
            btn.textContent = 'SUMMARIZING...';
            btn.style.opacity = '0.4';

            var transcript = curMessages.map(function(m, i) {
                var role = m.role === 'user' ? 'User' : (curEnt.nickname || curEnt.name);
                var text = m.isDirector ? ('[SCENE] ' + (m.scene||'') + ' [DLG] ' + (m.dlg||'')) : m.text;
                return '[' + (i+1) + '] ' + role + ': ' + text;
            }).join('\n');

            var cfg = activeCfg();
            if (!cfg.key) { btn.textContent = '✦ SUMMARIZE TO MEMORY'; btn.style.opacity = '.7'; return; }

            var now = new Date();
            var dateStr = now.getFullYear() + '-' + String(now.getMonth()+1).padStart(2,'0') + '-' + String(now.getDate()).padStart(2,'0');
            var prompt = '你是一个记忆存档系统。阅读以下 Lens 沉浸式互动记录，把发生的事情原原本本记下来。\n\n' +
                '时间：' + dateStr + '\n' +
                '场景：Lens 沉浸式叙事\n\n' +
                '【你的任务】\n' +
                '像写流水账日记一样，把每件事的起因、经过、结果完整写下来。不要总结，不要概括，不要美化。\n\n' +
                '【严禁】\n' +
                '- 禁止使用"似乎""可能""或许"等模糊词\n' +
                '- 禁止写"双方关系有所进展"这种废话，要写具体发生了什么\n' +
                '- 禁止省略任何肢体接触、情绪爆发、重要对话的原文\n' +
                '- 禁止用一句话概括多件事，一件事写一条\n\n' +
                '【格式】\n' +
                '- 用 [user] 代替用户，[char] 代替角色\n' +
                '- 每行一条，前缀 HIGH: / MID: / LOW:\n' +
                '- 每条必须是完整的一句话或一段话，包含：谁、做了什么、为什么、对方什么反应\n' +
                '- 重要对话直接引用原话，用引号括起来\n\n' +
                '【分级】\n' +
                'HIGH: 改变关系的事件、承诺、情感转折、不可逆行为、第一次做某事\n' +
                'MID: 有意义的互动、偏好暴露、身体接触细节、值得记住的话\n' +
                'LOW: 场景环境、临时状态、氛围\n\n' +
                '【示例（学这个写法）】\n' +
                'HIGH: [char] 主动握住了 [user] 的手腕，说"别走"，声音在发抖。这是 [char] 第一次主动肢体接触并表达挽留。\n' +
                'MID: [user] 说自己怕黑，[char] 没有嘲笑，而是把灯调亮了一些，假装是自己觉得太暗。\n' +
                'LOW: 场景在深夜的阳台，能听到远处的车声，两人之间隔着一根烟的距离。\n\n' +
                '只输出 HIGH:/MID:/LOW: 开头的记忆条目，不要输出任何其他文字。\n\n' +
                '对话记录：\n' + transcript;
            ...
            .then(function(data) {
                ...
                if (!text) { btn.textContent = '✦ FAILED'; setTimeout(function(){ btn.textContent = '✦ SUMMARIZE TO MEMORY'; btn.style.opacity = '.7'; }, 1500); return; }
                ...
                btn.textContent = '✓ ' + total + ' MEMORIES SAVED';
                setTimeout(function() { btn.textContent = '✦ SUMMARIZE TO MEMORY'; btn.style.opacity = '.7'; }, 2000);
            })
            .catch(function() {
        ...
        /* ── 轮数滑块 ── */
        document.getElementById('lensRoundsToggle').addEventListener('click', function () {
            document.getElementById('lensRoundsCard').classList.toggle('open');
        });
        document.getElementById('lensRoundsSlider').addEventListener('input', function () {
            document.getElementById('lensRoundsVal').textContent = this.value;
            localStorage.setItem('lens-context-rounds', this.value);
        });
        var savedRounds = localStorage.getItem('lens-context-rounds');
        if (savedRounds) {
            document.getElementById('lensRoundsSlider').value = savedRounds;
            document.getElementById('lensRoundsVal').textContent = savedRounds;
        }
        ...
        function getArchives() {
            if (!curId) return [];
            try { return JSON.parse(localStorage.getItem('lens-archives-' + curId) || '[]'); } catch(e) { return []; }
        }
        function saveArchives(arcs) {
            if (!curId) return;
            localStorage.setItem('lens-archives-' + curId, JSON.stringify(arcs));
        }

        function renderArchivePanel() {
            var arcs = getArchives();
            var statsEl = document.getElementById('lensArcStats');
            var scrollEl = document.getElementById('lensArcScroll');
            var totalMsgs = arcs.reduce(function(s, a) { return s + (a.messages ? a.messages.length : 0); }, 0);
            statsEl.innerHTML =
                '<div style="display:flex;flex-direction:column;gap:2px;"><div style="font-family:Syncopate,sans-serif;font-size:16px;font-weight:700;color:#fff;">' + arcs.length + '</div><div style="font-size:8px;letter-spacing:2px;color:rgba(255,255,255,0.25);text-transform:uppercase;">Archives</div></div>' +
                '<div style="display:flex;flex-direction:column;gap:2px;"><div style="font-family:Syncopate,sans-serif;font-size:16px;font-weight:700;color:#fff;">' + totalMsgs + '</div><div style="font-size:8px;letter-spacing:2px;color:rgba(255,255,255,0.25);text-transform:uppercase;">Messages</div></div>';

            if (arcs.length === 0) {
                scrollEl.innerHTML =
                    '<div style="text-align:center;padding:60px 30px;">' +
                        '<div style="width:56px;height:56px;margin:0 auto 16px;border-radius:50%;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;justify-content:center;"><svg viewBox="0 0 24 24" style="width:22px;height:22px;stroke:rgba(255,255,255,0.2);fill:none;stroke-width:1.5;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div>' +
                        '<div style="font-family:Playfair Display,serif;font-size:16px;font-style:italic;color:rgba(255,255,255,0.5);margin-bottom:6px;">No archives yet</div>' +
                        '<div style="font-size:11px;color:rgba(255,255,255,0.25);line-height:1.6;">Save your current conversation to create an archive, then start fresh.</div>' +
                    '</div>';
                return;
            }
            ...
            arcs.forEach(function(arc, idx) {
                var msgCount = arc.messages ? arc.messages.length : 0;
                var preview = '';
                if (arc.messages && arc.messages.length) {
                    var lastAi = null;
                    for (var i = arc.messages.length - 1; i >= 0; i--) {
                        if (arc.messages[i].role === 'assistant') { lastAi = arc.messages[i]; break; }
                    }
                    if (lastAi) preview = (lastAi.text || '').substring(0, 80).replace(/\[.*?\]/g, '').trim();
                }
                var date = arc.createdAt ? new Date(arc.createdAt).toLocaleDateString() : '';
                var num = String(idx + 1).padStart(2, '0');

                html += '<div style="position:relative;margin-bottom:14px;cursor:pointer;transition:transform 0.25s;" data-arc-idx="' + idx + '">' +
                    '<div style="position:absolute;top:-8px;left:16px;width:60px;height:16px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-bottom:none;border-radius:6px 6px 0 0;display:flex;align-items:center;justify-content:center;"><span style="font-family:Syncopate,sans-serif;font-size:6px;letter-spacing:2px;color:rgba(255,255,255,0.3);text-transform:uppercase;">ARC ' + num + '</span></div>' +
                    '<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:18px;position:relative;overflow:hidden;">' +
                        '<div style="position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,rgba(180,210,255,0.15),transparent);"></div>' +
                        '<div style="position:absolute;bottom:-4px;right:8px;font-family:Playfair Display,serif;font-style:italic;font-size:36px;color:rgba(255,255,255,0.015);letter-spacing:2px;line-height:1;pointer-events:none;">Archive</div>' +
                        '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">' +
                            '<div style="font-family:Noto Serif SC,serif;font-size:14px;color:rgba(255,255,255,0.85);font-weight:500;line-height:1.3;">' + esc(arc.name || 'Untitled') + '</div>' +
                            '<div style="font-size:9px;color:rgba(255,255,255,0.25);flex-shrink:0;">' + date + '</div>' +
                        '</div>' +
                        '<div style="display:flex;gap:12px;margin-bottom:10px;">' +
                            '<div style="display:flex;align-items:center;gap:4px;padding:3px 8px;background:rgba(255,255,255,0.04);border-radius:4px;"><svg viewBox="0 0 24 24" style="width:10px;height:10px;stroke:rgba(255,255,255,0.3);fill:none;stroke-width:2;"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg><span style="font-size:9px;color:rgba(255,255,255,0.35);">' + msgCount + ' msgs</span></div>' +
                            '<div style="display:flex;align-items:center;gap:4px;padding:3px 8px;background:rgba(255,255,255,0.04);border-radius:4px;"><svg viewBox="0 0 24 24" style="width:10px;height:10px;stroke:rgba(255,255,255,0.3);fill:none;stroke-width:2;"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg><span style="font-size:9px;color:rgba(255,255,255,0.35);">Heat ' + (arc.heat || 0) + '%</span></div>' +
                        '</div>' +
                        (preview ? '<div style="font-family:Cormorant Garamond,serif;font-size:12px;font-style:italic;color:rgba(255,255,255,0.3);line-height:1.6;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;border-bottom:1px dashed rgba(255,255,255,0.12);padding-bottom:8px;">' + esc(preview) + '</div>' : '') +
                        '<div style="position:absolute;top:14px;right:14px;display:flex;gap:6px;">' +
                            '<div class="lens-arc-load" data-arc-idx="' + idx + '" style="width:24px;height:24px;border-radius:50%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg viewBox="0 0 24 24" style="width:10px;height:10px;stroke:rgba(255,255,255,0.5);fill:none;stroke-width:2;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg></div>' +
                            '<div class="lens-arc-del" data-arc-idx="' + idx + '" style="width:24px;height:24px;border-radius:50%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg viewBox="0 0 24 24" style="width:10px;height:10px;stroke:rgba(240,80,80,0.7);fill:none;stroke-width:2;"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></div>' +
                        '</div>' +
                    '</div>' +
                '</div>';
            });
            ...
            scrollEl.querySelectorAll('.lens-arc-load').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.stopPropagation();
                    var ai = parseInt(btn.dataset.arcIdx, 10);
                    var arcs2 = getArchives();
                    var arc = arcs2[ai];
                    if (!arc || !arc.messages) return;
                    if (!confirm('Load this archive? Current conversation will be replaced.')) return;
                    curMessages = arc.messages.slice();
                    saveConv(curId, curMessages);
                    heat = arc.heat || 0;
                    document.getElementById('lensHeatSlider').value = heat;
                    updateHeat(heat);
                    document.getElementById('lensArchivePanel').style.display = 'none';
                    reRenderMessages();
                });
            });

            scrollEl.querySelectorAll('.lens-arc-del').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.stopPropagation();
                    var ai = parseInt(btn.dataset.arcIdx, 10);
        ...
        document.getElementById('lensSbNewChapter').addEventListener('click', function() {
            document.getElementById('lensSb').classList.remove('open');
            if (!curId || !curMessages.length) return;
            pushAndSave({ role: 'system', text: '__CHAPTER_BREAK__', isChapterBreak: true });
            var container = document.getElementById('lensChatContainer');
            var typing = document.getElementById('lensTyping');
            var divider = document.createElement('div');
            divider.className = 'msg-row';
            divider.style.cssText = 'display:flex;align-items:center;gap:12px;padding:20px 24px;';
            divider.innerHTML = '<div style="flex:1;height:1px;background:linear-gradient(to right,transparent,rgba(255,255,255,0.15),transparent);"></div>' +
                '<div style="font-family:Syncopate,sans-serif;font-size:7px;letter-spacing:3px;color:rgba(255,255,255,0.25);text-transform:uppercase;white-space:nowrap;">New Chapter</div>' +
                '<div style="flex:1;height:1px;background:linear-gradient(to right,transparent,rgba(255,255,255,0.15),transparent);"></div>';
        ...
        document.getElementById('lensSbClear').addEventListener('click', function() {
            document.getElementById('lensSb').classList.remove('open');
            if (!curId) return;
        ...
        document.getElementById('lensSbArchive').addEventListener('click', function() {
            document.getElementById('lensSb').classList.remove('open');
            document.getElementById('lensArchivePanel').style.display = 'flex';
            renderArchivePanel();
        });

        document.getElementById('lensArcClose').addEventListener('click', function() {
            document.getElementById('lensArchivePanel').style.display = 'none';
        });

        document.getElementById('lensArcSave').addEventListener('click', function() {
            if (!curId || !curMessages.length) return;
            var name = prompt('Give this archive a name:', 'Scene · ' + new Date().toLocaleDateString());
            ...
            var saveBtn = document.getElementById('lensArcSave');
            var origHTML = saveBtn.innerHTML;
            saveBtn.innerHTML = '<svg viewBox="0 0 24 24" style="width:12px;height:12px;stroke:currentColor;fill:none;stroke-width:2;"><polyline points="20 6 9 17 4 12"/></svg>Saved!';
            setTimeout(function() { saveBtn.innerHTML = origHTML; }, 1500);
        });
    }
